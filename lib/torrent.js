    ...
    return function (cb) {
      self.storage.read(piece.index, function (err, buffer) {
        if (!err && buffer) {
          ...
          // without breaking piece encapsulation but still supporting buffer-backed and
          // fs-backed storage.
          var oldBuffer = piece.buffer
          piece.buffer = buffer
          piece.verify()
          piece.buffer = oldBuffer
        }
        cb()
      }, true)
    }
...
Torrent.prototype._onStorage = function () {
  ...
  process.nextTick(function () {
    // TODO: we should emit separate 'metadata' and 'ready' events, where:
    //    'metadata' signifies the torrent metadata has been parsed and is ready.
    //    'ready' signifies the metadata, swarm, and storage are all fully initialized
    //    and the torrent is ready to begin transferring.
    self.emit('metadata')
  })
}
