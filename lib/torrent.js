...
Torrent.prototype.select = function (start, end, priority, notify) {
  ...
  if (start > end || start < 0 || end >= self.storage.pieces.length) {
    return console.error("invalid selection ", start, ":", end)
  }
...
Torrent.prototype._onMetadata = function (metadata) {
  ...
  self.storage = new Storage(self.parsedTorrent, { log : self.log })
...
Torrent.prototype._gcSelections = function () {
  ...
  for (var i = 0; i < self._selections.length; i++) {
    ...
    while (self.storage.bitfield.get(s.from + s.offset) && s.from + s.offset < s.to) s.offset++
...
Torrent.prototype._updateTick = function () {
  var self = this
...
Torrent.prototype._update = function () {
  ...
  // TODO/IDEA: iterate through wires in random order each _update for better request distribution?
  self.swarm.wires.forEach(function (wire) {
  ...
  function speedRanker () {
    ...
    return function(index) {
      ...
      for (; ptr < self.swarm.wires.length; ptr++) {
        ...
        if (otherSpeed <= speed || !otherWire.peerPieces.get(index)) continue
  ...
  function trySelectWire(hotswap) {
    ...
    for (var i = 0; i < self._selections.length; i++) {
      ...
      for (var j = next.from + next.offset; j <= next.to; j++) {
        ...
        while (wire.requests.length < MAX_OUTSTANDING_REQUESTS && self._request(wire, j, self._critical[j] || hotswap))
...
Torrent.prototype._hotswap = function (wire, index) {
  var self = this
  if (!self.hotswapEnabled) return
  
  var speed = wire.downloadSpeed()
  if (speed < Storage.BLOCK_LENGTH) return
  if (!self._reservations[index] || self.storage.bitfield.get(index)) return
  ...
  for (var i = 0; i < minWire.requests.length; i++) {
    ...
    self.storage.cancelBlock(index, (req.offset / Storage.BLOCK_SIZE) | 0)
  }

  engine.emit('hotswap', minWire, wire, index)
  return true
}
...
Torrent.prototype._request = function (wire, index, hotswap) {
  ...
  if (!block && hotswap && !endGame && self._hotswap(wire, index))
    block = self.storage.reserveBlock(index, endGame)
