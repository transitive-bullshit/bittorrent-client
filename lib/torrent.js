...
Torrent.prototype._onWireWithMetadata = function (wire) {
  ...
  function requestPiece (index) {
    ...
    self.log(wire.remoteAddress, 'requestPiece', index, 'offset', block.offset, 'length', block.length)
    wire.request(index, block.offset, block.length, function (err, buffer) {
      if (err) {
        ...
        if (!success) {
          self.log(wire.remoteAddress, 'requestPiece', index, 'offset', block.offset, 'length', block.length, 'ERROR writing block')
          self.storage.deselectBlock(index, block.offset)
        } else {
          requestPieces()
        }
