...
Torrent.prototype._onWire = function (wire) {
  ...
  wire.on('timeout', function () {
    debug('wire', wire.remoteAddress, 'timed out')
...
Torrent.prototype._onMetadataAndSwarm = function () {
  ...
  self.storage = new self._storageImpl(self.parsedTorrent)
  ...
  self.storage.on('done', function () {
    debug()
    debug('DONE DOWNLOADING TORRENT!')
    ...
    parallel(self.storage.pieces.map(function (piece) {
      return function (cb) {
        self.storage.read(piece.index, function (err, buffer) {
          ...
          //debug('piece', piece.index, err, buffer && buffer.length)
          if (!err && buffer) {
            // TODO: this is a bit hacky; figure out a cleaner way of verifying the buffer
            piece.verify(buffer)
            numVerified += piece.verified
            debug('piece ' + piece.index + ' ' + (piece.verified ? 'verified' : 'invalid'))
          }
...
Torrent.prototype._onStorage = function () {
  ...
  process.nextTick(function () {
    ...
    // TODO: if storage emits 'done' then we already make torrent emit 'done' as well.
...
Torrent.prototype._request = function (wire, index, hotswap) {
  ...
  function gotPiece (err, buffer) {
    ...
    if (err) {
      debug(wire.remoteAddress, 'requestPiece', index, 'offset', block.offset, 'length', block.length, 'ERROR', err)
      self.storage.cancelBlock(index, block.offset)
      process.nextTick(self._update.bind(self))
      return false
    } else {
      debug(wire.remoteAddress, 'requestPiece', index, 'offset', block.offset, 'length', block.length, 'DONE')
      self.storage.writeBlock(index, block.offset, buffer, function (err) {
        if (err) {
          debug(wire.remoteAddress, 'requestPiece', index, 'offset', block.offset, 'length', block.length, 'ERROR writing block', err)
          self.storage.cancelBlock(index, block.offset)
        }
