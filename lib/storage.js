...
Piece.prototype.readBlock = function (offset, length, cb) {
  ...
  if (!self._verifyOffset(offset)) {
    process.nextTick(function () {
      cb(new Error('invalid block offset ' + offset))
    })
    return
  }
  process.nextTick(function () {
...
Piece.prototype.writeBlock = function (offset, buffer, cb) {
  var self = this
  if (!self._verifyOffset(offset, true) || !self._verifyBlock(offset, buffer)) {
    process.nextTick(function () {
      cb(new Error('invalid block ' + offset + ':' + buffer.length))
    })
    return
  }
...
Piece.prototype.verify = function () {
  var self = this
  if (self.verified || !self.buffer) {
    return
  }
...
Piece.prototype._verifyOffset = function (offset, forceAlloc) {
  var self = this
  if (offset % BLOCK_LENGTH === 0) {
    if (!self.buffer) {
      if (forceAlloc) self._lazyAllocBuffer()
      else return false
    }
...
Storage.prototype.read = function (index, range, cb, force) {
  ...
  if (!piece.verified && !force) {
    return process.nextTick(function () {
      cb(new Error('Storage.read called on incomplete piece ' + index))
    })
  }
  ...
  if (piece.buffer) {
    ...
    return process.nextTick(function () {
      cb(null, piece.buffer.slice(offset, offset + length))
    })
  }
