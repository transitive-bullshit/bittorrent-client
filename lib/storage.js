...
Piece.prototype._verify = function () {
  ...
  if (self.verified) {
    ...
    self.emit('error', new Error('piece ' + self.index + ' failed verification; ' + sha1(self.buffer) + ' expected ' + self.hash))
    self._reset()
  }
}
...
Piece.prototype._verifyOffset = function (offset) {
  if (offset % BLOCK_LENGTH === 0) {
    ...
    self.emit('error', new Error('piece ' + self.index + ' invalid offset ' + offset + ' not multiple of ' + BLOCK_LENGTH + ' bytes'))
    return false
  }
}
...
Piece.prototype._verifyBlock = function (offset, buffer) {
  ...
  if ((self.length - offset) < BLOCK_LENGTH || buffer.length === BLOCK_LENGTH) {
    ...
    self.emit('error', new Error('piece ' + self.index + ' invalid block of size ' + buffer.length + ' bytes'))
    return false
  }
}
 ...
 */
function File (storage, file, buffer, pieces, pieceLength) {
  var self = this
  EventEmitter.call(self)
