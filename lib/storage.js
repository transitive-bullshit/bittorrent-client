...
function Storage (parsedTorrent) {
  ...
  self.lastPieceLength = parsedTorrent.lastPieceLength
  ...
  self.pieces = self.parsedTorrent.pieces.map(function (hash, index) {
    ...
    var end = start + (index === numPieces - 1 ? self.lastPieceLength : self.pieceLength)
  ...
  self.files = self.parsedTorrent.files.map(function (fileObj) {
    ...
    var startPiece = Math.floor(start / self.pieceLength)
    var endPiece = Math.floor((end - 1) / self.pieceLength)
...
Storage.prototype._onPieceDone = function (piece) {
  ...
  console.log('PIECE DONE', piece.index, '(' + self.numMissing + ' missing)')
  self.emit('piece', piece)
}

Storage.prototype._onFileDone = function (file) {
  var self = this
  console.log('FILE DONE', file.name, '(' + self.numMissing + ' missing)')
  self.emit('file', file)
